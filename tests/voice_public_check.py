...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'docs/evidence/phase7'
manifest=json.loads((OUT/'deployment-manifest.json').read_text());report=[]
private=['/'+r['source'] for r in manifest['files'] if r['source'].startswith('chat/')]+['/chat/chat.js?v=20260908-chat4','/fire/shaders.js','/api/stats.php']
...
checks={r['path']:r['after'] for r in manifest['files']};checks.update({'/var/www/dashboard/'+p:h for p,h in manifest['preserved'].items()})
