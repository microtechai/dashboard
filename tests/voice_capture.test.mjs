...
test('worklet resamples real input; max four unacked frames then fails closed',()=>{
 ...
 const p=new Processor();for(let i=0;i<80;i++)p.process([[new Float32Array(128).fill(.25)]]);
 assert.equal(sent.filter(x=>x.type==='frame').length,4);assert.equal(sent.filter(x=>x.type==='error').length,1);
