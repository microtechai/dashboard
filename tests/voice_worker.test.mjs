...
test('worker rejects frame 17 rather than overlap or accumulate inference',async()=>{
 ...
 for(let i=0;i<16;i++)self.onmessage({data:{type:'frame',frame:new Float32Array(512)}});
 assert.equal(out.filter(m=>m.type==='error').length,0);
 self.onmessage({data:{type:'frame',frame:new Float32Array(512)}});
