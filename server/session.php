...
function fail(int $status, string $message): never { reply(['error' => $message], $status); }
...
function authCall(string $path, ?array $body = null): array {
    global $config;
    $ch = curl_init(rtrim($config['auth_base'], '/') . $path);
    $cookie = null; $response = '';
    $headers = ['Accept: application/json'];
    if (isset($_SESSION['mc_token'])) $headers[] = 'Cookie: session=' . $_SESSION['mc_token'];
    if ($body !== null) $headers[] = 'Content-Type: application/json';
    curl_setopt_array($ch, [CURLOPT_CONNECTTIMEOUT => 3, CURLOPT_TIMEOUT => 8,
