"""Selective phase7 rollback, --check by default. --apply explicitly restores only manifest files."""
from pathlib import Path
import json,subprocess,sys
ROOT=Path(__file__).resolve().parents[1]
manifest=json.loads((ROOT/'docs/evidence/phase7/deployment-manifest.json').read_text())
...
assert str(backup).startswith('/root/jarvis-voice7-rollback-')
for r in m['files']:
 p=Path(r['path']);assert str(p).startswith('/var/www/dashboard/chat/') or str(p)=='/opt/jarvis-access/gate.php'
...
if apply:
 for r in reversed(m['files']):
  ...
  if r['before'] is None:p.unlink()
   ...
   tmp=p.with_name(p.name+'.voice7-restore');shutil.copy2(backup/'before'/r['source'],tmp);os.chmod(tmp,r['mode']);os.chown(tmp,r['uid'],r['gid']);os.replace(tmp,p)
...
print(r.stdout)
if not apply:(ROOT/'docs/evidence/phase7/rollback-check.json').write_text(r.stdout)
